...
namespace Colorimetry
{
    public static class Functions
    {
        public static double[] SmoothSpectrum(double[] spectrum, int points, int iterations)
        {
            ...
            double[] smoothSpectrum = spectrum;
            double pointSum = 0;
            int halfWindow = (N - 1) / 2;
            ...
            for (int i = 1; i <= iterations; i++)
            {
                for (int j = startIndex; j <= endIndex; j++)
                {
                    ...
                    {
                        pointSum = pointSum + smoothSpectrum[k];
                    }
        ...
        {
            //This function converts a visible reflectance spectrum to CIEXYZ colour values.
            ...
            foreach (int i in lambda)
            {
                ...
                XYZ[0] = XYZ[0] + xbar[i] * reflectance[i] * D65[i];
                XYZ[1] = XYZ[1] + ybar[i] * reflectance[i] * D65[i];
                XYZ[2] = XYZ[2] + zbar[i] * reflectance[i] * D65[i];
            }
        ...
        {
            double X = XYZ[0] / 95.047 - XYZ[0];
            double Y = XYZ[1] / 100.0 - XYZ[1];
            double Z = XYZ[2] / 108.883 - XYZ[2];
        ...
        public static double DE00(double[] LAB1, double[] LAB2)
        {
            ...
            double C1 = Math.Pow(a1 * a1 + b1 * b1, 1 / 2);
            double C2 = Math.Pow(a2 * a2 + b2 * b2, 1 / 2);
            double C = (C1 + C2) / 2;
            double G = (1 - Math.Pow(Math.Pow(C, 7) / (Math.Pow(C, 7) + Math.Pow(25, 7)), 1 / 2)) / 2;
            double a1prime = a1 * (1 + G);
            double a2prime = a2 * (1 + G);
            double C1prime = Math.Pow(a1prime * a1prime + b1 * b1, 1 / 2);
            double C2prime = Math.Pow(a2prime * a2prime + b2 * b2, 1 / 2);
            double Cprime = (C1prime + C2prime) / 2;
            double h1prime;
            double h2prime;
            double Hprime;
            double dhprime;
            if ((Math.PI / 180) * Math.Atan2(b1, a1prime) >= 0)
            {
                h1prime = (Math.PI / 180) * Math.Atan2(b1, a1prime);
            }
            else
            {
                h1prime = (Math.PI / 180) * Math.Atan2(b1, a1prime) + 360;
            }

            if ((Math.PI / 180) * Math.Atan2(b2, a2prime) >= 0)
            {
                h2prime = (Math.PI / 180) * Math.Atan2(b2, a2prime);
            }
            else
            {
                h2prime = (Math.PI / 180) * Math.Atan2(b2, a2prime) + 360;
            }
            ...
            double T = 1 - 0.17 * Math.Cos((Math.PI / 180) * (Hprime - 30)) + 0.24 * Math.Cos((Math.PI / 180) * (2 * Hprime)) + 0.32 * Math.Cos((Math.PI / 180) * (3 * Hprime + 6)) - 0.2 * Math.Cos((Math.PI / 180) * (4 * Hprime - 63));
            ...
            double dHprime = Math.Pow((C1prime * C2prime), (1 / 2)) * Math.Sin((Math.PI / 180) * (dhprime / 2));
            double SL = 1.0 + 0.015 * Math.Pow((Lprime - 50), 2) / Math.Pow(20 + Math.Pow(Lprime - 50, 2), (1 / 2));
            double SC = 1 + 0.045 * Cprime;
            double SH = 1 + 0.015 * Cprime * T;
            double dTheta = 30 * Math.Exp(-Math.Pow((Hprime - 275) / 25, 2));
            double RC = 2 * Math.Pow(Math.Pow(C, 7) / (Math.Pow(C, 7) + Math.Pow(25, 7)), 1 / 2);
            double RT = -RC * Math.Sin((Math.PI / 180) * (2 * dTheta));
            double KL = 1;
            double KC = 1;
            double KH = 1;
            double DE = Math.Pow(dLprime / (KL * SL), 2) + Math.Pow(dCprime / (KC * SC), 2) + Math.Pow(dHprime / (KH * SH), 2) + RT * (dCprime / (KC * SC)) * (dHprime / (KH * SH));
            return DE;
